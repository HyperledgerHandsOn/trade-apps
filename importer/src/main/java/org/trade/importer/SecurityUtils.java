...
public class SecurityUtils implements Serializable {
	...
	protected static final String bankerRole = "BANKER";
	protected static final String clientRole = "CLIENT";

	protected static boolean recognizedRole(String role) {
		role = role.toUpperCase();
		return (role.equals(adminRole) || role.equals(bankerRole) || role.equals(clientRole));
	}
	...
	protected static boolean isBanker(Authentication authResult) {
		Collection<? extends GrantedAuthority> auths = authResult.getAuthorities();
		for (GrantedAuthority auth: auths) {
			if (auth.getAuthority().equals("ROLE_" + bankerRole)) {
					return true;
			}
		}
		return false;
	...
	protected static boolean isImporter(Authentication authResult) {
		Collection<? extends GrantedAuthority> auths = authResult.getAuthorities();
		for (GrantedAuthority auth: auths) {
