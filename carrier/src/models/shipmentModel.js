...
class ShipmentManager {
	...
	async acceptShipmentAndIssueBL(tradeId, blId, expirationDate, sourcePort, destinationPort, cb) {
		logger.info('Invoking acceptShipmentAndIssueBL: Accepting a shipment of goods and issuing a bill of lading.');
		try {
			await this.shipmentContract.submitTransaction('acceptShipmentAndIssueBL', tradeId, blId, expirationDate, sourcePort, destinationPort);
	...
	async updateShipmentLocation(tradeId, location, cb) {
		logger.info('Invoking updateShipmentLocation: Updating a shipment\'s location.');
		try {
			await this.shipmentContract.submitTransaction('updateShipmentLocation', tradeId, location);
