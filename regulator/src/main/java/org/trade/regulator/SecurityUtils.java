...
public class SecurityUtils implements Serializable {
	...
	protected static boolean isAdmin(Authentication authResult) {
		Collection<? extends GrantedAuthority> auths = authResult.getAuthorities();
		for (GrantedAuthority auth: auths) {
				if (auth.getAuthority().equals("ROLE_" + adminRole)) {
						return true;
				}
		}
		return false;
	...
	protected static boolean isClient(Authentication authResult) {
		Collection<? extends GrantedAuthority> auths = authResult.getAuthorities();
		for (GrantedAuthority auth: auths) {
